...
│  │  └──────────────────────────────────────────┘        │        │
│  └────────────────────────────────────────────────────────┘        │
│  Files: infrastructure/02-redis/                                   │
│  Purpose: Cache hot data, worker communication, presence           │
└─────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────┐
│  OBJECT STORAGE: MinIO (S3-compatible)                             │
│  ┌────────────────────────────────────────────────────────┐        │
│  │  Distributed MinIO - EC:4 (Erasure Coding)            │        │
│  │  ┌──────┐  ┌──────┐  ┌──────┐  ┌──────┐             │        │
│  │  │ Node │  │ Node │  │ Node │  │ Node │             │        │
│  │  │  1   │  │  2   │  │  3   │  │  4   │             │        │
│  │  └──────┘  └──────┘  └──────┘  └──────┘             │        │
...
┌─────────────────────────────────────────────────────────────────────┐
│  NETWORKING: Ingress + TLS + Security                              │
│  ┌────────────────────────────────────────────────────────┐        │
...
│  Files: main-instance/02-element-web/                              │
│  Purpose: User interface for chat, accessible via browser          │
├────────────────────────────────────────────────────────────────────┤
│  coturn (TURN/STUN)           │  NAT traversal for calls           │
...
┌─────────────────────────────────────────────────────────────────┐
│                   USER INTERFACES FOR LI                        │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Element Web LI                                          │  │
│  │  • Connects to Synapse LI                                │  │
│  │  • Shows deleted messages in UI                          │  │
│  │  • Read-only interface                                   │  │
│  │  Files: li-instance/02-element-web-li/                   │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Synapse Admin LI                                        │  │
│  │  • Admin interface for forensics                         │  │
│  │  • User search, room inspection                          │  │
│  │  • Full audit trail access                               │  │
│  │  Files: li-instance/03-synapse-admin-li/                 │  │
│  └──────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
...
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Dashboards:                                             │  │
│  │  • Synapse Overview (messages, users, rooms)             │  │
│  │  • Worker Performance (per-worker metrics)               │  │
...
│  Purpose: Visual dashboards for system health                  │
└─────────────────────────────────────────────────────────────────┘
...
┌──────────────────────────────────────────────────────────────────┐
│                    NGINX Ingress                                 │
│            Routes /_matrix/media/* to Content Scanner            │
└───────────────────────────┬──────────────────────────────────────┘
                            │
                            ▼
┌─────────────────────────────────────────────────────────────────┐
...
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  1. Receive media upload/download request               │  │
│  │  2. Check cache: Was this file already scanned?         │  │
│  │     ├─ Yes, Clean → Serve immediately                   │  │
│  │     ├─ Yes, Infected → Return HTTP 403                  │  │
│  │     └─ No → Continue to step 3                          │  │
│  │  3. Forward request to Synapse                          │  │
│  │  4. Scan file with ClamAV                               │  │
...
│   │
│   ├── 02-grafana/
│   │   └── dashboards-configmap.yaml → Pre-built dashboards
...
│   ├── minio-operator-values.yaml   → MinIO operator
│   ├── metallb-values.yaml          → Load balancer IPs
│   ├── nginx-ingress-values.yaml    → Ingress controller
│   ├── cert-manager-values.yaml     → TLS certificate manager
...
└── docs/                       ← Reference Documentation
    │
    ├── 00-WORKSTATION-SETUP.md        → REQUIRED: Setup kubectl, helm
    ...
    ├── HAPROXY-ARCHITECTURE.md        → Optional: HAProxy routing details
...
8. POSTGRESQL LI REPLICA
   ...
   └─ Event now visible in LI instance
        ...
        ▼
9. GENERIC WORKER
   └─ Get event ID from Event Persister
   └─ Return success to client
   └─ Notify other clients via /sync endpoint
        │
        ▼
10. SYNC WORKERS
    └─ Connected clients receive message update
    └─ Element Web displays message
...
1. USER INITIATES CALL
   └─ Element Web requests TURN credentials
        │
        ▼
2. SYNAPSE MAIN
   └─ Generate TURN credentials using shared secret
   └─ Return credentials to client
        │
        ▼
3. CLIENTS ESTABLISH PEER CONNECTION
   └─ Try direct connection (STUN)
   └─ If NAT prevents direct: Use TURN relay
        │
        ▼
4. COTURN (TURN Server)
   └─ Relay media packets between clients
   └─ Authenticate using credentials from step 2
        │
        ▼
5. MEDIA STREAMING
   └─ WebRTC peer-to-peer (or via TURN)
   └─ Audio/video flows between clients
        │
        ▼
6. CALL EVENTS
...
└────────────────────────────────────────────────────────────────────────────────────────┘
                ...
                ▼                      ▼                      ▼
        ┌──────────────┐      ┌──────────────┐      ┌──────────────┐
        │ Element Web  │      │   HAProxy    │      │Content Scanner│
        │   (Main)     │      │ (Routing)    │      │   + ClamAV   │
        └──────────────┘      └──────┬───────┘      └──────┬────────┘
                                     │                     │
                ┌────────────────────┼─────────────────────┘
