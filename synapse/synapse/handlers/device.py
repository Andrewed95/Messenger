...
class DeviceHandler:
    ...
    def __init__(self, hs: "HomeServer"):
        ...
        if hs.config.registration.max_sessions_per_user is not None:
            self.session_limiter = SessionLimiter(
                hs.config.registration.max_sessions_per_user
            )
            logger.info(
                f"LI: Session limiter initialized with max {hs.config.registration.max_sessions_per_user} sessions per user"
            )
    ...
    async def check_device_registered(
        ...
        # LI: Check session limit before creating device (if device_id is known)
        if device_id is not None and self.session_limiter:
        ...
        while attempts < 5:
            ...
            # LI: Check session limit for autogenerated device_id
            if self.session_limiter:
                can_create = self.session_limiter.check_can_create_session(
                    user_id=user_id,
            ...
            if new_device:
                # LI: Add session after successful device creation
                if self.session_limiter:
                    success = self.session_limiter.add_session(user_id, new_device_id)
                    if success:
                        logger.info(f"LI: Session added for {user_id}/{new_device_id}")
